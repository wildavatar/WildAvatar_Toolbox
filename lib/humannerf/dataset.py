...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.cameras, self.mesh_infos = self.load_metadata()
        framelist = self.load_train_frames()
        self.framelist = framelist[start::skip]
        if maxframes > 0:
    ...
    def load_canonical_joints(self):
        ...
        cl_joint_path = 'datasets/canonical_joints.pkl'
    ...
    def load_metadata(self):
        ...
        mesh_infos = {}
        all_betas = []
        for frame_base_name in tqdm.tqdm(frame_infos):
            cam_body_info = frame_infos[frame_base_name] 
            imgname = cam_body_info['imgname']
            poses = np.array(cam_body_info['poses'], dtype=np.float32)
            betas = np.array(cam_body_info['betas'], dtype=np.float32).reshape(-1)
    ...
    def __getitem__(self, idx):
        ...
        # vising joints
        # joints3d = self.mesh_infos[frame_name]['joints']
        ...
        #     cv2.circle(this_img, (int(joint[0]), int(joint[1])), 5, (255,0,0), 5)
        # print("ok")
